...
public sealed class ReleaseEngineeringTests
{
    ...
    public void App_project_uses_v0632_beta_version_metadata()
    {
        ...
        Assert.Contains("<Version>0.6.39-beta</Version>", project);
        Assert.Contains("<FileVersion>0.6.39.0</FileVersion>", project);
        Assert.Contains("<AssemblyVersion>0.6.39.0</AssemblyVersion>", project);
        Assert.Contains("<InformationalVersion>0.6.39 Beta</InformationalVersion>", project);
    }
    ...
    public void Release_documents_describe_cover_update_and_local_data()
    {
        ...
        var releaseDraft = File.ReadAllText(FindRepositoryFile("docs", "github-release-v0.6.39-beta.md"));

        Assert.Contains("v0.6.39 Beta", readme);
        Assert.Contains("GestureClip-v0.6.39-beta-win-x64.zip", readme);
        ...
        Assert.Contains("GestureClip v0.6.39 Beta", changelog);
        Assert.Contains("GestureClip-v0.6.39-beta-win-x64.zip", releaseDraft);
