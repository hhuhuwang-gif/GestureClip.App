...
{
    [Fact]
    public void App_project_uses_v0613_beta_version_metadata()
    {
        var project = File.ReadAllText(FindRepositoryFile("src", "GestureClip.App", "GestureClip.App.csproj"));

        Assert.Contains("<Version>0.6.13-beta</Version>", project);
        Assert.Contains("<FileVersion>0.6.13.0</FileVersion>", project);
        Assert.Contains("<AssemblyVersion>0.6.13.0</AssemblyVersion>", project);
        Assert.Contains("<InformationalVersion>0.6.13 Beta</InformationalVersion>", project);
    }
    ...
    public void Release_documents_describe_cover_update_and_local_data()
    {
        ...
        var releaseDraft = File.ReadAllText(FindRepositoryFile("docs", "github-release-v0.6.13-beta.md"));

        Assert.Contains("v0.6.13 Beta", readme);
        Assert.Contains("GestureClip-v0.6.13-beta-win-x64.zip", readme);
        ...
        Assert.Contains("GestureClip v0.6.13 Beta", changelog);
        Assert.Contains("GestureClip-v0.6.13-beta-win-x64.zip", releaseDraft);
